...
Contains
  ...
  Subroutine init_hitline_netcdf(fname,nhitline)
    ...
    integer :: varid_line_index, varid_r, varid_z, varid_phi
    ...
    ierr = nf90_def_var(ncid, "line_index", NF90_INT, [dimid_snapshot], varid_line_index)
    If (ierr /= NF90_NOERR) Then
       Write(*,*) nf90_strerror(ierr)
       Stop
    End If
  ...
  subroutine write_hitline_data_netcdf(fname, line_index, r_hitline, z_hitline, phi_hitline)
    use netcdf
    ...
    integer(int32), intent(in) :: line_index
    ...
    integer(int32) :: line_index_out(1)
    ...
    ierr = nf90_inq_varid(ncid, "line_index", varid_line_index)
    If (ierr /= NF90_NOERR) Then
       Write(*,*) nf90_strerror(ierr)
       Stop
    End If
    ...
    line_index_out(1) = line_index

    ! write data
    ierr = nf90_put_var(ncid, varid_line_index, line_index_out, start=[snapshot_len+1], count=[1])
    If (ierr /= NF90_NOERR) Then
       Write(*,*) nf90_strerror(ierr)
       Stop
    End If
