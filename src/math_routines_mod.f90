...
Contains
  ...
  Subroutine spline_fit(n, x, y, y2)
    ...
    Implicit None
    ! Computes second derivatives (y2) for natural cubic spline interpolation
    Integer(int32), Intent(in) :: n
    Real(real64), Intent(in)   :: x(n), y(n)
    Real(real64), Intent(out)  :: y2(n)

    Integer(int32) :: i, k
    Real(real64)   :: p, qn, sig, un
    Real(real64), Allocatable :: u(:)

    Allocate(u(n-1))

    ! Set boundary conditions for a natural spline
    y2(1) = 0.0_real64
    u(1) = 0.0_real64
    ...
    Do i = 2, n - 1
       ...
       p = sig * y2(i-1) + 2.0_real64
       y2(i) = (sig - 1.0_real64) / p
       u(i) = (6.0_real64 * ((y(i+1)-y(i))/(x(i+1)-x(i)) - (y(i)-y(i-1))/(x(i)-x(i-1))) &
    ...
    y2(n) = (un - qn*u(n-1))/(qn*y2(n-1) + 1.0_real64)
  ...
  !------------------------------------------------------------------------------
  Function spline_eval(n, xa, ya, y2a, x) Result(y)
    Use kind_mod, Only : int32, real64
    Implicit None    
    ! Evaluates the cubic spline interpolation at a given point x
    Integer(int32), Intent(in) :: n
    Real(real64), Intent(in)   :: xa(n), ya(n), y2a(n)
    ...
    ! Optional check: Ensure input grid is strictly increasing
    If (Any(xa(2:n) <= xa(1:n-1))) Stop 'spline_eval: xa array must be strictly increasing.'
    ...
    h = xa(khi) - xa(klo)
    If (h == 0.0_real64) Stop 'spline_eval: bad xa input, zero interval width.'

    a = (xa(khi) - x) / h
    b = (x - xa(klo)) / h

    y = a * ya(klo) + b * ya(khi) + &
         ((a**3 - a)*y2a(klo) + (b**3 - b)*y2a(khi)) * (h*h)/6.0_real64
  ...
  Subroutine line_seg_facet_int_old(pa,pb,pc,p1,p2,ithit,p,tol,calc_theta,sin_theta)
    ...
    If (abs(denom) .lt. tol) Then
       ...
       if ((mu .lt. 0._real64) .or. (mu .gt. 1._real64)) Then
          ithit = 0
       else
  ...
  Subroutine line_seg_facet_int(pa,pb,pc,p1,p2,ithit,p,tol,calc_theta,sin_theta)
    ...
    !local variables and arrays
    Real(real64), Dimension(3) :: n, v
    Real(real64), Dimension(3) :: e1, e2, ap
    Real(real64) :: d, denom, mu, n2
    Real(real64) :: mag_v, dot_nv
    ...
    If (abs(denom) .lt. tol) Then
       ...
       if ((mu .lt. -tol) .or. (mu .gt. 1._real64 + tol)) Then
          ithit = 0
       else
          p(1) = p1(1) + mu * (p2(1) - p1(1))
          p(2) = p1(2) + mu * (p2(2) - p1(2))
          ...
          ! Determine whether the intersection point is bounded by pa,pb,pc
